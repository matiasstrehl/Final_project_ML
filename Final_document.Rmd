...
In this section, I proceed to asses the models' performance based on a more sophisticated re-sampling approach, known as cross validation, in order to get a better estimation of the true MSE. To do that, I split the training set into 5 folds. Note that I am fitting 3 models per each fold, so I am fitting a total of 15 models.
...
Notice that following the ROC-AUC criterium, the best model is one with really low amount of regularization and with the lowest possible proportion of Lasso Penalty (Ridge regression). Now that we have chosen the best-performing model, we can proceed to fit that model to the whole training set, and then, just as an exercise to compare in the future, we can use it to predict redistribution preferences in the testing set. This does not mean that we are choosing this one as the best-performing model of the paper. In fact, we will see that tree-based models do a slightly better prediction job. Therefore, fitting the model to the testing set is only an illustrative exercise.

```{r reg-test, message = F, warning = F}
# Update/finalize recipe using the best model
en_final <- finalize_workflow(en_wkflow, best_model)

# Fit the best model to the whole training set
en_final_fit <- fit(en_final, data = data_train)

# Apply the model to the testing set
predicted_data <- augment(en_final_fit, new_data = data_test) %>%
  select(pref_redistribution, starts_with(".pred"))

# Table with assessment
acc_en <- predicted_data %>%
  accuracy(truth = pref_redistribution, estimate = .pred_class) # accuracy

acc_table <-  acc_en$.estimate[1] %>%
  as_tibble() %>%
  rename(Accuracy = value)

roc_auc_en <- predicted_data %>%
  roc_auc(truth = pref_redistribution, estimate = .pred_favor) # ROC-AUC

roc_auc_table <- roc_auc_en$.estimate[1] %>%
  as_tibble() %>%
  rename("ROC-AUC" = value)

en_table <- bind_cols(acc_table, roc_auc_table) %>%
            mutate_if(is.numeric, format, digits = 3)

kbl(en_table,
    caption = "<center> Assessment of the best regularized model in the testing set",
    booktabs = TRUE,
    escape = FALSE,
    align = "cc") %>%
  kable_classic(full_width = F)
...
In Table \@ref(tab:reg-test), we observe that the best-regularized model correctly predicts 74.6% of the observations in the testing set, while it achieves a ROC-AUC of 0.8. We can also plot the ROC-AUC curve.

```{r reg-roc, message = F, warning = F, fig.cap="ROC-AUC curve of best-regularized model on testing data"}
...
```{r reg-matrix, message = F, warning = F, fig.cap="Confusion matrix of regularized model in the testing data" }
...
Figure \@ref(fig:reg-matrix) shows the confusion matrix of the best-regularized model on the testing data. By looking at this matrix, we observe that among those observations that are in favor of more redistribution (891), the model correctly predicts 63.1% (562), and among those observations that are against more redistribution (1337), the model correctly predicts 82.2% (1099).
...
#predicted_test %>%
#  roc_auc(truth = pref_redistribution, estimate = .pred_favor)
...
models_name <- c("Regularized model", "Random Forest") %>%
  as_tibble() %>%
  rename(Model = value)
...
final_table <- bind_cols(models_name, final_rows)

kbl(final_table,
    caption = "<center> Assessment of the models in the testing set",
    booktabs = TRUE,
    escape = FALSE,
    align = "lcc") %>%
  kable_classic(full_width = F)
...
We observe that the ROC-AUC of the best random forest model in the testing set is of 0.806, while it achieves an accuracy of 74.6%. Compared to the regularized logistic regression that we fitted to the testing set in Section 3, the random forest model does a slightly better job in both accuracy and ROC-AUC level in the testing set. Figure \@ref(fig:roc-test) plots the ROC-AUC curve of the random forest model for a complete analysis.
...
```
Figure \@ref(fig:matrix-test) presents the confusion matrix of this model in the testing set. We observe that, among those that are in favor of redistribution in the testing set (891), the model correctly predicts 60% of them (540). On the other hand, among those that are against redistribution in the testing set (1337), the model correctly predicts 84.5% of them (1130). Compared to the exercise we did when we fitted the regularized logistic regression to the testing set, the random forest does a relatively worst job in predicting those that are in favor, but a better job in predicting those that are against redistribution in the testing set.
...
```
Overall, the random forest model does a slightly better job in predicting redistribution preferences in the testing set compared to the regularized logistic regression. The random forest model was the best-performing model in the folds, and it is, therefore, the best-performing model of the paper. We compare its performance in the testing set with the best regularized model just as an illustrative exercise.
...
As a final exercise, I fitted the random forest model to the testing set and, only as an exercise, I also fitted the regularized logistic model. Overall, the random forest model achieves an accuracy of 75% and a ROC-AUC of 0.806. 

As a final comment, given that this model does a slightly better job in predicting redistribution preferences, both in terms of accuracy and ROC-AUC level in the testing set, relative to the regularized LOGISTIC model, the gains of performing a random forest model are not obvious. This is because random forest models are computationally-demanding compared to the simpler regularized logistic models, and in this exercise, the gains in terms of performance are not significant in magnitude. Even though the random forest performed slightly better, both models had a similar performance, so a simpler regularized LOGISTIC regression does almost as good job as the random forest in prediciting redistribution preferences in the present study.
